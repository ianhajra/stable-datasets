...
def test_mosquito_sound_dataset():
    ...
    assert series.shape == (3750, 1), f"series should have shape (3750, 1), got {series.shape}"
    ...
    assert 0 <= label < 6, f"label should be between 0 and 5, got {label}."

    # Test 5: test split size
    ds_test = MosquitoSound(split="test")
    expected_num_test_samples = 139883
    assert len(ds_test) == expected_num_test_samples, (
        f"Expected {expected_num_test_samples} test samples, got {len(ds_test)}."
    )

    print("All MosquitoSound dataset tests passed successfully!")
